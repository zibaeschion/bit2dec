...
export default function DecimalAndBinaryCounter(props) {
    const [decimalDigitVisible, setDecimalDigitVisible] = React.useState([
        true,
        true,
    ]);
    const [binaryDigitVisible, setBinaryDigitVisible] = React.useState([
        true,
        true,
        true,
        true,
    ...
    const increaseDecimalDigit = (digitIndex) => {
        if (!(digitIndex >= decimalNumber.length || digitIndex < 0)) {
            if (decimalNumber[digitIndex] < 9) {
                setDecimalNumber((array) =>
                    array.with(digitIndex, decimalNumber[digitIndex] + 1)
                );
            } else {
    ...
    return (
        <Grid2 container spacing={2} columns={13}>
            ...
            <Grid2 size={4}>
                <Paper
                    ref={containerRefDecimal}
                    sx={{
                        overflow: 'hidden',
                        backgroundColor: prefersDarkMode ? '#3c3c3c' : '',
                        color: prefersDarkMode ? '#fff' : '',
                    }}
                >
                    <Stack direction="row" justifyContent={'space-evenly'}>
                        <Slide
                            in={decimalDigitVisible[0]}
                            direction={slideDirection}
                            container={containerRefDecimal.current}
            ...
            <Grid2 size={8}>
                ...
                >
                    <Stack direction="row" justifyContent={'space-evenly'}>
                        <Slide
                            in={binaryDigitVisible[0]}
                            direction={slideDirection}
                            container={containerRefBinary.current}
                            timeout={slideTimeout}
                        >
                            <Typography
                                color={colorForBinaryNumbers}
